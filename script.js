...
function addLayer() {
    ...
    newLayer.innerHTML = `
        <div class="layer-main">let scene, camera, renderer, controls;
let threeDCanvas;
let nameTextAlign = 'left';
let thicknessTextAlign = 'right';
...
function drawSection() {
    ...
    this.timeout = setTimeout(() => {
        ...
        const ctx = canvas.getContext('2d', { 
            alpha: false,
            willReadFrequently: false  // 読み取り操作を行わないことを明示
        });
        ...
        // 描画前に変換を一度だけ計算
        const layers = Array.from(document.getElementsByClassName('input-group'));
        const { scale, startY, startX, drawableWidth } = calculateDrawingParams(layers, height, width);
        ...
        // 描画をバッファリング
        layers.forEach(layer => {
            const { height: layerHeight, color, texts } = calculateLayerParams(layer, scale);
            ...
            // テキスト描画（条件を満たす場合のみ）
            if (layerHeight > 20 && texts) {
                drawLayerTexts(ctx, texts, startX, currentY, drawableWidth, layerHeight);
            }

            currentY += layerHeight;
        });
    }, 32);  // 30FPSに制限（パフォーマンス改善）
}

// 描画用パラメータの計算を分離
function calculateCanvasSize() {
    const width = window.innerWidth <= 768 ? 
        Math.min(window.innerWidth - 40, 800) : 800;
    const height = window.innerWidth <= 768 ? 
        Math.min(window.innerHeight - 200, 600) : 600;
    return { width, height };
}

// レイヤーパラメータの計算を分離
function calculateLayerParams(layer, scale) {
    ...
    return {
        ...
        color: layer.querySelector('.layer-color').value,
        texts: getLayerTexts(layer, rawThickness, unit)
    };
}
